...
def get_embedder():
    """Return an embedding function that produces 3046-d vectors.

    We force local embeddings to a model with 3046 dimensions to avoid
    ...
    logger.info(f"Using local 3046-d embeddings: {model}")
    ...
    try:
        ...
        if EMBED_DIM != 3046:
            logger.warning(f"Expected 3046-d embeddings, got {EMBED_DIM}. Ensure model outputs 3046-d vectors.")
    except Exception:
        EMBED_DIM = None
    return emb
...
def index_from_urls(file_path: Path):
    _indexer.index_from_urls_file(file_path)
...
def main():
    ...
    if args.print_compose:
        print_compose(); return
    if args.print_env:
        print_env_example(); return
    if args.init_db:
        init_db(); return
    if args.index:
        if args.local:
